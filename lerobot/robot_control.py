...
from lerobot.common.policies.diffusion.modeling_diffusion import DiffusionPolicy, DiffusionModel
...
episode_index = 1  # ← 任意のエピソード番号に変更可能
...
initial_joint = torch.tensor(joint_states[0][0], dtype=torch.float32)
...
print(f"最初に {n_obs_steps} 個の観測データを集める")
...
for i in range(6):
    axes[i].plot(pred_actions_np[:, i], label='action Prediction', linestyle='--')
    axes[i].plot(obs_joint_np[:, i], label='ground_truth', linestyle='-')
...
plt.suptitle(f"action_prediction_&_obs_joint_data[{episode_index}]")
plt.tight_layout()
prediction_plot_path = os.path.join(save_root, f'action_prediction_&_obs_joint_data[{episode_index}].png')
